...
export const applyForJob = TryCatch(async (req: AuthenticatedRequest, res) => {
  ...
  VALUES (${jobId}, ${user.user_id}, ${user.email}, ${resume}, ${isSubscribed})`;
  } catch (error: any) {
    if (error.code === "23505") {
...
export const getAllApplications = TryCatch(
  async (req: AuthenticatedRequest, res) => {
    const [applications] = await sql`SELECT a.*, j.title AS job_title, j.salary AS job_salary, j.location AS job_location FROM applications a JOIN jobs j ON a.job_id = j.job_id WHERE applicant_id = ${req.user?.user_id}`
