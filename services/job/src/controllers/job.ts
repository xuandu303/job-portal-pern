...
export const getCompanyDetails = TryCatch(async (req: AuthenticatedRequest, res) => {
  ...
  const [companyData] = await sql`SELECT c.*, COALESCE ((SELECT json_agg(j.*) FROM jobs j WHERE j.company_id = c.company_id), '[]'::json) AS jobs FROM companies c WHERE c.company_id = ${id}`;
...
export const getAllActiveJobs = TryCatch(async (req: AuthenticatedRequest, res) => {
  ...
  let queryString = `SELECT j.job_id, j.title, j.description, j.salary, j.location, j.job_type, j.role, j.work_location, j.created_at, c.name AS company_name, c.logo AS company_logo, c.company_id FROM jobs j JOIN companies c ON j.company_id = c.company_id WHERE j.is_active = true`
